...
export class Enemies{
    ...
    takeDamage(hero){
        const x1 = hero.x +1,x2=hero.x-1,y=hero.y;
        const i =this._enemies.findIndex((value)=>(value.x===x1 || value.x===x2)&&value.y===y);
        if(i == -1)return;
        ...
        this._intervals[i] = setInterval(()=>{
            ...
            if(hero.status === "died" || (enemy.x-1)>hero.x||(enemy.x+1)<hero.x||enemy.y!==hero.y || enemy.status === "died"){
